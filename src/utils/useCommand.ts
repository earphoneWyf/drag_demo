...
export const useCommand = (totalData: any, focusData: any) => {
    ...
    register({
        ...
        init() {
            this.before = null;
            //监听拖拽开始事件，保存状态
            const start = () => this.before = deepcopy(totalData.blocks);
            //拖拽之后需要触发对应的指令
        ...
        execute() {
            ...
            let after = totalData.blocks;
            return {
                //默认一松手直接把当前事情做了
                redo() {
                    totalData.blocks = after;
                    // totalData= { ...totalData, blocks: after }
                },
                //前一步
                undo() {
                    totalData.blocks = before;
                    // totalData = { ...totalData, blocks: before }
                }
            }
        }
    });
    ...
    register({
        ...
        execute(newval:Data) {
            let state = {
                before: totalData,//当前的值
                after: newval,//新值
            }

            return {
                redo() {
                    totalData.container=state.after.container;
                    totalData.blocks=state.after.blocks;
                },
                undo() {
                    totalData.container=state.before.container;
                    totalData.blocks=state.before.blocks;
                }
            }
        }
    })
    ...
    register({
        ...
        pushQueue: true,
        execute() {
            let before = deepcopy(totalData.blocks);
            //置顶就是在所有的block张找到最大的
            let after = (() => {
    ...
    register({
        ...
        execute() {
            let state = {
                before: deepcopy(totalData.blocks),//当前的值
                after: focusData.value.unfocused,//选中的都删除了，留下的都是没选中的
            }
            return {
                redo() {
                    totalData.blocks = state.after;
                },
                undo() {
                    totalData.blocks = state.before;
                }
            }
        }
    })
    ...
    register({
        ...
        execute(newBlock:Block, oldBlock:Block) {
            let state =reactive( {
                before: totalData.blocks,
                after: (() => {
                    let blocks = deepcopy(totalData.blocks);//拷贝一份用于新的block
                    let index=blocks.findIndex((obj:any)=>JSON.stringify(obj)===JSON.stringify(oldBlock));//找老的 需要通过老的查找
                    if (index > -1) {
